...
export class SpaceController {
  static async createSpace(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      if (!req.user) {
  ...
  static async updateSpace(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      if (!req.user) {
        res.status(401).json({ error: 'Authentication required' });
        return;
  ...
  static async deleteSpace(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      if (!req.user) {
        res.status(401).json({ error: 'Authentication required' });
        return;
  ...
  static async getUserSpaces(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      if (!req.user) {
        res.status(401).json({ error: 'Authentication required' });
        return;
